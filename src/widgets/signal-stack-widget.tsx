...
export const signalStackWidget = createWidget<SignalStackWidgetProps, SignalStackWidgetConfiguration>(
  ...
  (props, environment) => {
    ...
    // limitRows are already fairly distributed and capped by widget-sync
    // (see applyFairLimitRowCaps there) — keep this flatten dead simple because
    // the expo-widgets SwiftUI transform can't handle loops/helpers here.
      ...
      <VStack
        ...
        {isLarge && hasLimits ? (
          <VStack alignment="leading" spacing={2} modifiers={[frame({ maxWidth: 10000, alignment: "leading" })]}>
            {flatLimitRows.slice(0, 13).map((row) => {
              const ratio = Math.max(0, Math.min(1, row.ratio));
          ...
          <VStack alignment="leading" spacing={5}>
            {hasLimits
              ? flatLimitRows.slice(0, isLarge ? 13 : 3).map((row) => {
